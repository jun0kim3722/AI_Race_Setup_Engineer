...
class ldChan(object):
    ...
    def data(self):
        ...
        skip_list = [
             'LAP_BEACON', 'TIME', 'GEAR', 
             'BUMPSTOPUP_RIDE_LF', 'BUMPSTOPUP_RIDE_RF', 
             'BUMPSTOPUP_RIDE_LR', 'BUMPSTOPUP_RIDE_RR',
             'BUMPSTOPDN_RIDE_LF', 'BUMPSTOPDN_RIDE_RF',
             'BUMPSTOPDN_RIDE_LR', 'BUMPSTOPDN_RIDE_RR'
            ]
        if self._data is None:
            ...
            with open(self._f, 'rb') as f:
                ...
                try:
                    ...
                    if self.name not in skip_list:
                        self._data = smooth((self._data/self.scale * pow(10., -self.dec) + self.shift) * self.mul, self.freq)
...
def smooth(data, freq, time_step=0.2, polyorder=3):
    window = int(time_step // (1.0 / freq))
...
def read_ldfile(f_):
    ...
    try:
        ...
        raise FileNotFoundError(".ld file is not found")
    return head_, chans
